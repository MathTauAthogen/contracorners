...
class Frame {
	...
	public:
		...
		bool draw_on ( std::vector < Drawable* > drawable );
};
...
inline bool Frame::draw_on ( std::vector < Drawable* > drawables )
{
	bool changed_anything = false, is_changed = false;

	typename std::vector < Drawable* >::iterator count = drawables.begin(), end = drawables.end();
